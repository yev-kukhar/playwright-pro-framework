...
test.describe('PrivatBank API Tests', () => {
  ...
  test.describe('Currency Exchange Rates', () => {
    ...
    test('should validate decimal format of rates', async () => {
      ...
      rates.forEach(rate => {
        ...
        const buyDecimals = rate.buy.split('.')[1]?.length || 0;
        const saleDecimals = rate.sale.split('.')[1]?.length || 0;
        
        expect(buyDecimals).toBeLessThanOrEqual(2);
        expect(saleDecimals).toBeLessThanOrEqual(2);
      });
    });
  });
  ...
  test.describe('Historical Exchange Rates', () => {
    ...
    test('should get exchange rates for specific date', async () => {
      ...
      console.log(`Historical rates for ${testDate}: ${response.exchangeRate.length} currencies`);
    });

    test('should contain major currencies in historical data', async () => {
      const response = await privatBankService.getExchangeRatesByDate('01.12.2023');
    ...
    test('should have valid rate structure', async () => {
      const response = await privatBankService.getExchangeRatesByDate('01.12.2023');
      const firstRate = response.exchangeRate[0];
      ...
      expect(firstRate).toHaveProperty('saleRate');
      expect(firstRate).toHaveProperty('purchaseRate');
